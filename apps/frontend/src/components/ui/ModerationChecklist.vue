<template>
  <div class="card moderation-checklist">
    ...
    <div v-else-if="generatedMessage">
      <p>在此输入您的审核消息。请记得检查审核标签，以回答作者可能提出的任何问题！</p>
      ...
      <div v-else>
        ...
        >
          <p v-if="modPackData[modPackIndex].status === 'unidentified'">
            该项目是否提供了
            <strong>{{ modPackData[modPackIndex].file_name }}</strong> 的识别和权限？
          </p>
  ...
  [
    {
      ...
      question: "标题是否没有无用信息？",
      shown: true,
      rules: ["没有不必要的数据（模组加载器、游戏版本等）", "没有表情符号/无用的文本装饰"],
      examples: [
...
async function generateMessage() {
  ...
  if (modPackData.value && modPackData.value.length > 0) {
    ...
    if (
      ...
      printMods(unidentifiedMods, "以下内容无法识别。请提供其来源的证明以及您有权包含它的证明：");
      message.value += "\n\n";
    }
  }
